...
function Lvl2() {
  ...
  async function initializeGame() {
    const { word, letterObject } = getRandomWord(6)
    setCorrectWord(word)
  ...
  function handleEnter() {
    if (currentWord === correctWord) {
      setGameOver(true)
      navigate("/lvl3", { state: { result: "win" } })
      return
    }

    if (currentWord !== correctWord && wordCount === TOTAL_GUESSES - 1) {
      setGameOver(true)
      navigate("/score", { state: { result: 2 } })
      return
    }

    if (letterCount !== WORD_LEN) {
      alert("words must be 6 letters")
      return
    }

    if (!isValidGuess(currentWord, 6)) {
