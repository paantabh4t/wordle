...
function Lvl4() {
  ...
  async function initializeGame() {
    const { word, letterObject } = getRandomWord(8)
    setCorrectWord(word)
  ...
  function handleEnter() {
    if (currentWord === correctWord) {
      setGameOver(true)
      navigate("/lvl5", { state: { result: "win" } })
      return
    }

    if (currentWord !== correctWord && wordCount === TOTAL_GUESSES - 1) {
      setGameOver(true)
      navigate("/score", { state: { result: 4 } })
      return
    }

    if (letterCount !== WORD_LEN) {
      alert("words must be 8 letters")
      return
    }

    if (!isValidGuess(currentWord, 8)) {
